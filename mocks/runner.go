...
func (m *MockRunner) Run(v0 config.Config) <-chan error {
	...
	m.RunFunc.history = append(m.RunFunc.history, RunnerRunFuncCall{v0, r0})
	return r0
}
...
func (m *MockRunner) Shutdown(v0 time.Duration) error {
	...
	m.ShutdownFunc.history = append(m.ShutdownFunc.history, RunnerShutdownFuncCall{v0, r0})
	return r0
}
...
// PushHook adds a function to the end of hook queue. Each invocation of the
// Shutdown method of the parent MockRunner instance inovkes the hook at the
// front of the queue and discards it. After the queue is empty, the default
// hook function is invoked for any future action.
